...
class YieldHandler(QCHandler):
    ...
    def check_qc(self):
        ...
        for lane_dict in self.conversion_results:
            ...
            if self.error() != self.UNKNOWN and lane_yield < float(self.qc_config["error"]) * pow(10, 9):
                yield QCErrorFatal("Yield was to low on lane {}, it was: {}".format(lane_nbr, lane_yield))
            elif self.warning() != self.UNKNOWN and lane_yield < float(self.qc_config["warning"]) * pow(10, 9):
